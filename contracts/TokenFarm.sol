        ...
        if (uniqueTokensStaked[msg.sender] == 0) {
            for (
                ...
                if (stakers[stakersIndex] == msg.sender) {
                    popStaker(stakersIndex);
                }
            }
        }
    }

    function popStaker(uint256 _stakerIndex) internal {
        require(_stakerIndex < stakers.length);
        stakers[_stakerIndex] = stakers[stakers.length - 1];
        stakers.pop();
    }
